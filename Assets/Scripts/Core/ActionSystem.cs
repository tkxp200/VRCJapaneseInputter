...
public class ActionSystem : MonoBehaviour
{
    ...
    private ulong rightGrubActionHandle = 0;
    private ulong leftGrubActionHandle = 0;
    ...
    void Start()
    {
        ...
        rightGrubActionHandle = ActionUtil.GetActionHandlePath($"/actions/ControllerInput/in/RightGrubAction");
        leftGrubActionHandle = ActionUtil.GetActionHandlePath($"/actions/ControllerInput/in/LeftGrubAction");
    ...
    void UpdateAction()
    {
        ...
        if (overlaySystem.GetOverlayVisible())
        {
            InputDigitalActionData_t grubResult;
            if (mainSystem.GetTrackHand() == ETrackedControllerRole.RightHand)
            {
                grubResult = ActionUtil.GetDigitalActionData(rightGrubActionHandle);
            }
            else
            {
                grubResult = ActionUtil.GetDigitalActionData(leftGrubActionHandle);
            }
            overlaySystem.GrubEvent(grubResult.bState, grubResult.bChanged);
        }
    }
