...
namespace OverlayVRUtil
{
    public static class OverlaySystemUtil
    {
        ...
        {
            if(OpenVR.System is not null) return;
    ...
    public static class OverlayUtil
    {
        ...
        public static SteamVR_Utils.RigidTransform GetTransformAbsolute(ulong handle)
        {
            ETrackingUniverseOrigin origin = default;
            HmdMatrix34_t matrix = default;
            var err = OpenVR.Overlay.GetOverlayTransformAbsolute(handle, ref origin, ref matrix);
            EVRErrThrowException(err, "オーバーレイの位置取得に失敗しました");
            return new SteamVR_Utils.RigidTransform(matrix);
        }
